"""Interactive Slack channel/DM resolver for TimeFinder."""
        ...
        with urllib.request.urlopen(req) as response:
            ...
            for conv in data.get("channels", []):
                if conv.get("is_mpim"):
                    ...
                    dm_list.append({"dm_id": conv["id"], "user_id": conv["user"]})
                else:
                    channel_map[conv["name"]] = conv["id"]

            return channel_map, dm_list, mpim_list
    except Exception as exc:
        print(f"Network error during conversation sync: {exc}")
        return None, None, None
...
def run_add_slack_channels() -> None:
    """Interactive resolver loop for channels, DMs, and group DMs."""
    print("--- TimeFinder Slack Target Resolver ---")
    ...
    while True:
        ...
        if not raw_input or raw_input.lower() in {"done", "quit", "exit"}:
            break
        ...
        for item in lookups:
            if "+" in item or "&" in item:
                ...
                if not found_group:
                    print("\r  Not Found:     No active Group DM found matching exactly those members.")
    ...
    if confirm in ["y", "yes"]:
        ...
        try:
            ...
            print(f"Failed to write configuration file to disk: {exc}")
    else:
