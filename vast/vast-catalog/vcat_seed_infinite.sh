...
#
# DEPENDENCIES: bash, fpart, ffp_sync, rsync, util-linux (setsid), wamerican
#
# ==============================================================================
# REVISION HISTORY:
...
while true; do
    ...
    # 2. Dispatch workers silently into terminal-detached sessions
	setsid -w fpsync -n 32 -v "$SRC_WS1" "${DEST_WS1}/" > "$LOG_WS1" 2> >(grep -v "can't access tty" >> "$LOG_WS1") &
    pid_ws1=$!
    
	setsid -w fpsync -n 32 -v "$SRC_WS2" "${DEST_WS2}/" > "$LOG_WS2" 2> >(grep -v "can't access tty" >> "$LOG_WS2") &
    pid_ws2=$!
