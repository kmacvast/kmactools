...
# Description: Interactive, menu-driven launcher for vast-opstat. When the tool
...
# Version: 1.0.0
################################################################################
"""Interactive launcher for vast-opstat.

Design notes:
- The wizard never re-implements flag rules. It emits an argv list that the
  caller runs through ``vast-opstat.parse_args`` + ``dispatch``; that keeps a
...
def _build_argv(ans):
    """Translate collected answers into a vast-opstat argv list (no secrets)."""
